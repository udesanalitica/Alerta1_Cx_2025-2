...
::: {.callout-note title="Descripción para todos los campus"}
En relación con el dominio de una segunda lengua extranjera, el 12,5% de los estudiantes manifiestan dominar el idioma inglés.
:::
...
# Mostrar información de diagnóstico primero
#cat("=== DIAGNÓSTICO ===\n")
#cat("Columnas en el dataset que contienen 'idioma' o similares:\n")
cols_idiomas <- colnames(datos)[grepl("ingles|frances|portug|aleman|italiano|otra", colnames(datos), ignore.case = TRUE)]
if(length(cols_idiomas) > 0) {
  cat(paste(cols_idiomas, collapse = ", "), "\n")
} else {
  cat("No se encontraron columnas relacionadas con idiomas\n")
}

#cat("\nPrimeras filas de columnas relevantes:\n")
#if(length(cols_idiomas) > 0) {
#  print(head(datos[cols_idiomas], 10))
#}
...
if("Mensaje" %in% colnames(tabla_final)) {
  ...
  cat("No se generaron datos para la tabla\n")
}
```

## Habilidades de comunicación y Socialización

Los resultados revelan que el 48,6% de los estudiantes experimentan nerviosismo al hablar en público. Además, se encontró que el 87,3% de los estudiantes no han tomado cursos de oratoria.

En cuanto a la interacción social, el 90,6% de los estudiantes se sienten cómodos y se expresan abiertamente en su grupo de amigos. Además, el 48,6% de los estudiantes toman la iniciativa al conocer a nuevas personas, mientras que el 49,5% esperan que sean ellos quienes inicien la conversación.
