...
# Paso 1: Consolidar y limpiar texto desde la variable tipo_antc
texto <- paste(na.omit(datos$tipo_antc), collapse = " ")

# 🔧 Corregir palabras unidas conocidas
replacements <- list(
  "preservativ[oa]s?p[ií]ldora[s]?" = "preservativo píldora",
  "oralp[ií]ldora[s]?" = "oral píldora",
  "preservativoinyectable[s]?" = "preservativo inyectables",
  "preservativodiut" = "preservativo DIU"
)
for (pattern in names(replacements)) {
  texto <- gsub(pattern, replacements[[pattern]], texto, ignore.case = TRUE)
}
